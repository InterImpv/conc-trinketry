...
static void __nand(reg_unit *regs, instr_t *instr)
{
	if (instr->rA)
		regs->rx[instr->rA] =
			~(regs->rx[instr->rB] & regs->rx[instr->rC]);
...
enum GEN_ERR draw_instr(instr_t *instr)
{
	...
	case __NAND:
		mvprintw(Y_INSTR + 1, X_INSTR, "%s $r%d, $r%d, $r%d",
			 __op2str(instr->opcode), instr->rA, instr->rB,
			 instr->rC);
		break;
	case __ADDI:
	case __SW:
...
static enum GEN_ERR draw_memseg(mem_unit *mem, uint32_t xpos, uint32_t ypos,
				uint16_t start, uint16_t end, uint16_t pos)
{
	if (NULL == mem)
		return E_ARG;
	...
	for (i = start, j = 1; i <= end; i++, j++) {
		...
		if (isprint(mem->mem[i]) && isascii(mem->mem[i])) {
			mvprintw(ypos + j, xpos, "[0x%04x]: 0x%04x %c", i,
				 mem->mem[i], mem->mem[i]);
		} else {
			mvprintw(ypos + j, xpos, "[0x%04x]: 0x%04x", i,
				 mem->mem[i], mem->mem[i]);
		}
		attroff(A_BOLD);
	}
...
enum GEN_ERR draw_mem(mem_unit *mem)
{
	...
	draw_memseg(mem, X_ROM, Y_ROM, mem->rom_beginp, mem->rom_endp,
		    mem->rom_ptr);
	draw_memseg(mem, X_RAM, Y_RAM, mem->ram_beginp, mem->ram_endp,
		    mem->ram_ptr);
	return E_OK;
}
...
enum GEN_ERR draw_regs(reg_unit *regs)
{
	...
	for (int i = 0; i < N_OF_REGS; i++) {
		mvprintw(Y_REGS + i + 1, X_REGS, "$R%d: 0x%04x", i,
			 regs->rx[i]);
	}
